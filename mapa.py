...
grafica = open("GraficaSano.txt","w")
grafica.close()
grafica = open("GraficaS.txt","w")
grafica.close()
grafica = open("GraficaI.txt","w")
grafica.close()
grafica = open("GraficaR.txt","w")
grafica.close()
    ...
    if pasada > 1:
        ...
        for x in range(len(estados)):
            if pasada :
                ...
                if pinf < 30.0:
                    distribucion = np.random.choice(2,1,p=[0.8,0.2])
                    if distribucion[0] == 1 and auxestados[x] == "2":
                        auxestados [x] = "3"
                if pasada:
                    distribucion2 = np.random.choice(2,1,p=[0,1])
                    print distribucion2[0]
